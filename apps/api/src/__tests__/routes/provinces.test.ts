...
describe("provinces routes", async () => {
  let headers: Record<string, string> = {};
  beforeAll(async () => { headers = await authHeader(); });
  const app = createApp();

  test("returns all provinces", async () => {
    const response = await app.fetch(new Request("http://localhost/api/v1/provinces", { headers }));
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(Array.isArray(body)).toBe(true);
    expect(body.length).toBeGreaterThan(0);
    expect(body[0]).toMatchObject({ codigo: expect.any(String), nombre: expect.any(String), prefijo: expect.any(String) });
  });
});
