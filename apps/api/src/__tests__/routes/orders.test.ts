...
describe("orders routes", async () => {
  ...
  test("lists all orders", async () => {
    const response = await app.fetch(new Request("http://localhost/api/v1/orders", { headers }));
  ...
  test("lists current orders", async () => {
    const response = await app.fetch(new Request("http://localhost/api/v1/orders/current", { headers }));
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(Array.isArray(body)).toBe(true);
  });
