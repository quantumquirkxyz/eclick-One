...
import { comercioApiService } from "./services/commerceService";
...
export function App() {
  ...
  useEffect(() => { void Promise.all([comercioApiService.listarClientes(), comercioApiService.listarProductos(), comercioApiService.listarInventario(), comercioApiService.listarPedidos(), comercioApiService.listarPagos()]).then(([clientes, productos, inventario, pedidos, pagos]) => setDatos({ clientes, productos, inventario, pedidos, pagos })); }, []);
...
function Clientes({ clientes, pedidos }: { clientes: readonly Cliente[]; pedidos: readonly Pedido[] }) { return <section><Titulo titulo="Clientes" texto="Cartera, elegibilidad y actividad comercial" /><Tabla columnas={["Código", "Cliente", "Identificación", "Provincia", "Tarjeta", "Paz y salvo", "Pedidos"]} filas={clientes.map((cliente) => [String(cliente.codigo_cliente), `${cliente.nombre} ${cliente.apellido}`, cliente.identificacion, cliente.provincia.nombre, cliente.tipo_tarjeta, cliente.paz_y_salvo ? "Sí" : "No", String(pedidos.filter((pedido) => pedido.codigo_cliente === cliente.codigo_cliente).length)])} /></section>; }
function InventarioVista({ productos, inventario }: { productos: readonly Producto[]; inventario: readonly Inventario[] }) { return <section><Titulo titulo="Productos e inventario" texto="Disponibilidad, reserva y velocidad de consumo" /><div className="product-grid">{productos.map((producto) => { const item = inventario.find((value) => value.codigo_producto === producto.codigo_producto); if (!item) return null; const libres = item.cant_bodega - item.cant_reservado; const nivel = item.nivel_reposicion ?? 0; return <article className="product" key={producto.codigo_producto}><span>{libres <= nivel ? "Stock crítico" : `${libres} libres`}</span><h3>{producto.nombre}</h3><p>{producto.categoria} · Código {producto.codigo_producto}</p><div className="progress"><i style={{ width: `${Math.min(100, item.cant_bodega === 0 ? 0 : (item.cant_reservado / item.cant_bodega) * 100)}%` }} /></div><small>{item.cant_reservado} reservadas · {item.cant_bodega} en bodega</small></article>; })}</div></section>; }
function Pedidos({ pedidos, clientes, productos, compacto = false }: { pedidos: readonly Pedido[]; clientes: readonly Cliente[]; productos: readonly Producto[]; compacto?: boolean }) { const cliente = (codigo: number) => { const item = clientes.find((value) => value.codigo_cliente === codigo); return item ? `${item.nombre} ${item.apellido}` : "Cliente no encontrado"; }; const producto = (codigo: number) => productos.find((item) => item.codigo_producto === codigo)?.nombre ?? "Producto no encontrado"; return <section><Titulo titulo={compacto ? "Pedidos recientes" : "Pedidos"} texto={compacto ? "Muestra sintética" : "Ciclo de venta, pago y entrega"} /><Tabla columnas={["Pedido", "Cliente", "Producto", "Fecha", "Monto", "Estado"]} filas={pedidos.map((pedido) => [pedido.codigo_pedido, cliente(pedido.codigo_cliente), producto(pedido.codigo_producto), fecha.format(new Date(pedido.fecha_pedido)), dinero.format(pedido.monto), etiquetaEstado(pedido.estado)])} /></section>; }
function Pagos({ pagos }: { pagos: readonly Pago[] }) { return <section><Titulo titulo="Pagos" texto="Historial inmutable de cobros sintéticos" /><Tabla columnas={["ID", "Pedido", "Monto", "Tarjeta", "Fecha", "Referencia"]} filas={pagos.map((pago) => [String(pago.id_pago), pago.codigo_pedido, dinero.format(pago.monto_pagado), pago.tipo_tarjeta, fecha.format(new Date(pago.fecha_pago)), pago.referencia ?? "Sin referencia"])} /></section>; }
function Reportes({ pedidos, inventario }: { pedidos: readonly Pedido[]; inventario: readonly Inventario[] }) { const option = useMemo<EChartsOption>(() => ({ tooltip: { trigger: "axis" }, xAxis: { type: "category", data: inventario.map((item) => `Producto ${item.codigo_producto}`) }, yAxis: { type: "value" }, series: [{ type: "line", smooth: true, data: inventario.map((item) => item.cant_ventas), lineStyle: { color: "#0f766e", width: 3 }, itemStyle: { color: "#0f766e" } }], grid: { left: 45, right: 20, top: 25, bottom: 40 } }), [inventario]); return <section><Titulo titulo="Reportes" texto="Lectura ejecutiva del período permitido: días 1 al 30" /><div className="metrics"><Metric etiqueta="Pedidos incluidos" valor={String(pedidos.length)} nota="Datos sintéticos" /><Metric etiqueta="Preferencia" valor="Producto 1000" nota="Tres o más solicitudes" /></div><Panel titulo="Demanda por producto" subtitulo="Información de demostración"><Chart option={option} height={320} /></Panel></section>; }
