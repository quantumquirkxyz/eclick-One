...
describe("database migrations", () => {
  test("plans pending migrations without applying them in dry-run mode", async () => {
    ...
    expect(result.applied).toEqual(["001_initial_schema", "002_seed_reference_data"]);
  ...
  test("applies schema migrations idempotently", async () => {
    ...
    expect(await migrateUp(client, files)).toMatchObject({ applied: ["001_initial_schema"], dryRun: false });
