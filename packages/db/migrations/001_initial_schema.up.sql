...
SELECT
  p.id_pedido, p.codigo_pedido, p.fecha_solicitud, p.fecha_entrega, p.direccion_entrega, p.monto_esperado, p.estado,
  c.id_cliente, c.nombre AS cliente_nombre, c.apellido AS cliente_apellido, pr.id_provincia, pr.nombre AS provincia_nombre, pr.prefijo,
  pl.id_plan, pl.nombre AS plan_nombre, d.id_producto, d.cantidad,
...
SELECT
  p.id_pedido, p.codigo_pedido, p.fecha_solicitud, p.fecha_entrega, p.direccion_entrega, p.monto_esperado, p.estado,
  h.estado_anterior, h.estado_nuevo, h.fecha_cambio, c.id_cliente, pr.prefijo, pl.nombre AS plan_nombre, d.id_producto, d.cantidad,
  COALESCE(pg.estado_pago = 'APROBADO', 0) AS pagado
FROM PEDIDO p
JOIN CONTRATO ct ON ct.id_contrato = p.id_contrato
JOIN CLIENTE c ON c.id_cliente = ct.id_cliente
...
SELECT
  pg.id_pago, pg.id_pedido, p.codigo_pedido, pg.fecha_pago, pg.monto, pg.estado_pago, t.tipo_tarjeta, c.id_cliente,
  c.nombre AS cliente_nombre, c.apellido AS cliente_apellido, pl.nombre AS plan_nombre
FROM PAGO pg
